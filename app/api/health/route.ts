...
export async function GET() {
  ...
  if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    ...
    const [taskCheck, campaignCheck, auditCheck, limiterCheck] = await Promise.all([
  ...
  return Response.json({
    ...
    checks: { database, campaigns_migration: campaignsMigration, hardening_migration: hardeningMigration, required_configuration: missing.length === 0 },
    ...(ready ? {} : { action_required: { missing_environment_variables: missing, campaigns_migration: campaignsMigration ? 'applied' : 'apply supabase/migrations/004_campaigns.sql', hardening_migration: hardeningMigration ? 'applied' : 'apply supabase/migrations/005_production_hardening.sql' } }),
