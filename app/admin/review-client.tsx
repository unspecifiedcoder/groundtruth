...
type Lead = { id: string; source?: string; company_name?: string; contact_name?: string; work_email?: string; use_case?: string; launch_city?: string; estimated_locations?: number; timeline?: string; reply_url?: string; created_at: string }
...
export default function AdminReviewClient() {
  ...
  function exportLeads() {
    ...
    const fields: Array<keyof Lead> = ['created_at', 'source', 'company_name', 'contact_name', 'work_email', 'reply_url', 'launch_city', 'estimated_locations', 'timeline', 'use_case']
  ...
  return <main className="min-h-screen p-5"><div className="max-w-6xl mx-auto"><header className="flex flex-col sm:flex-row justify-between gap-4 mb-6"><div><p className="chip text-[10px] mb-2" style={{ color: 'var(--warn)' }}>Private operations</p><h1 className="font-display text-3xl font-extrabold">GroundTruth control room</h1><p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Prototype and pilot telemetry—never presented as customer traction.</p></div><div className="flex gap-2"><button className="btn btn-ghost px-4 py-2 text-sm" onClick={() => load()}>Refresh</button><button className="btn btn-ghost px-4 py-2 text-sm" onClick={logout}>Sign out</button></div></header>
    ...
    {tab === 'leads' && overview && <section><div className="flex justify-end mb-3"><button className="btn btn-ghost px-4 py-2 text-sm" onClick={exportLeads}>Export CSV</button></div>{overview.leads.length === 0 ? <Empty text="No pilot applications yet." /> : <div className="space-y-3">{overview.leads.map(lead => <article key={lead.id} className="card p-5"><div className="flex flex-col sm:flex-row justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-display text-lg font-bold">{lead.company_name ?? 'Unknown company'}</h2>{lead.source === 'a2a' && <span className="chip text-[9px]">A2A lead</span>}</div><p className="text-sm" style={{ color: 'var(--text-muted)' }}>{lead.contact_name}{lead.work_email && <> · <a className="underline" href={`mailto:${lead.work_email}`}>{lead.work_email}</a></>}{lead.reply_url && <> · <a className="underline" href={lead.reply_url} target="_blank" rel="noreferrer">Reply to agent</a></>}</p></div><span className="text-sm">{lead.estimated_locations ? `${lead.estimated_locations} locations` : 'Scope pending'}{lead.launch_city ? ` · ${lead.launch_city}` : ''}</span></div><p className="mt-3 text-sm whitespace-pre-wrap">{lead.use_case}</p></article>)}</div>}</section>}
